...
function compress() {
  if [ -z "$1" ] || [ -z "$2" ]; then
    echo "Usage: compress file_or_directory format"
  ...
  case $2 in
    tar.gz) tar czvf "${filename}.tar.gz" "$1" ;;
    tar.bz2) tar cjvf "${filename}.tar.bz2" "$1" ;;
    tar.xz) tar cJvf "${filename}.tar.xz" "$1" ;;
    zip) zip -r "${filename}.zip" "$1" ;;
    rar) rar a "${filename}.rar" "$1" ;;
    gz) gzip -k "$1" ;;
    tar) tar cvf "${filename}.tar" "$1" ;;
    tbz2) tar cjvf "${filename}.tbz2" "$1" ;;
    tgz) tar czvf "${filename}.tgz" "$1" ;;
    7z) 7z a "${filename}.7z" "$1" ;;
    *)
  ...
  echo "Compression complete: ${filename}.$2"
}
